...
For the bonus, I implemented a `build` function as an alternative to `new`. The key difference is in how they handle invalid input: `ThreadPool::new(size)` uses `assert!(size > 0)`, which panics the entire program if called with zero. In contrast, `ThreadPool::build(size)` returns a `Result<ThreadPool, PoolCreationError>`, allowing the caller to handle the error gracefully, for example, by logging it, falling back to a default size, or returning the error to the user. This makes bui`ld more robust for situations where the size might come from untrusted input like a configuration file or environment variable.

This naming convention actually follows a well-established pattern in the Rust standard library and ecosystem. Constructors named `new` are conventionally infallible (or panic on clearly programmer-error input), while fallible alternatives are often named `try_new`, `with_*`, or `build`. The builder pattern (e.g., `std::thread::Builder`, `std::process::Command`) uses `build`/`spawn` precisely because thread creation can realistically fail due to OS-level resource limits, and the caller deserves a chance to recover. It's a similar philosophy to `String::from_utf8` returning a `Result` instead of panicking on invalid bytes.

Comparing the two, `new` is more ergonomic when failure genuinely means "the programmer made a mistake", there's no meaningful way for a running program to recover from `ThreadPool::new(0)`, so panicking is acceptable. But `build` is safer when failure is a realistic runtime scenario that the caller might want to handle. In a real production server, I would lean toward `build`, because gracefully logging an error and exiting cleanly is almost always better than crashing with a panic. That said, I appreciate that Rust's type system makes the distinction explicit through the return type, just looking at a function signature tells you whether it can fail, which is much clearer than languages where any function might throw an unchecked exception.
